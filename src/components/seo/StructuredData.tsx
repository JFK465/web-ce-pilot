...
export function FAQSchema({ items }: FAQSchemaProps) {
  ...
  return (
    <script
      type="application/ld+json"
      // nosec: JSON-LD schema — FAQ items are static data, not user input
      dangerouslySetInnerHTML={{ __html: JSON.stringify(schema) }}
    />
  );
}
...
export function ArticleSchema({ post, url }: ArticleSchemaProps) {
  ...
  return (
    <script
      type="application/ld+json"
      // nosec: JSON-LD schema — article data is from static MDX files, not user input
      dangerouslySetInnerHTML={{ __html: JSON.stringify(schema) }}
    />
  );
}
...
export function BreadcrumbSchema({ items }: BreadcrumbSchemaProps) {
  ...
  return (
    <script
      type="application/ld+json"
      // nosec: JSON-LD schema — breadcrumb items are static route data, not user input
      dangerouslySetInnerHTML={{ __html: JSON.stringify(schema) }}
    />
  );
}
